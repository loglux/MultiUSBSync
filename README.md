...
Windows 10/11 (Windows PowerShell 5.1) - nothing extra to install.
...
| **Copy selected** | Runs the removal step (if anything's listed and confirmed), then copies the checked files to the checked drives. Two progress bars: overall (file N of M) and the current file's own byte-by-byte progress. |
